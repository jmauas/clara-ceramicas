...
export default function UpdatePage({ setShow, setForm}) {
    ...
    const submit = async (e) => {
        ...
        try {
            e.preventDefault();
            if (!data || !data.user || !data.user.email || !usuario.email) {
                setError('No se puede Modificar el Usuario')
                setLoad(false)
                return
            }
    ...
    useEffect(() => {
        if (data && data.user && data.user.perfil && data.user.perfil === 10) {
            setAdm(true)
        } else {
            setAdm(false)
        }
    ...
    return (
        <div>
            {usuario &&
            <>
            <h1 className="font-bold mb-5">Modificación de Datos de Usuario</h1>
