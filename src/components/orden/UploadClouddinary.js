...
export default function UploadClouddinary({ setScans, setImgs, texto, tipo, orden, user }) {
        ...
        if (result.event === 'success') {            
            ...
            if (tipo == 1) {
                if (format === 'png' || format === 'jpg' || format === 'jpeg' || format === 'gif' || format === 'webp' || format === 'svg' || format === 'bmp' || format === 'tiff' || format === 'ico') {
                    setImgs(prev => prev.concat({externa: true, nombre: original_filename, url: secure_url, public_id: public_id}))
                } else {
                    setScans(prev => prev.concat({externa: true, nombre: original_filename, url: secure_url, public_id: public_id}))
                }
            } else {
                if (format === 'png' || format === 'jpg' || format === 'jpeg' || format === 'gif' || format === 'webp' || format === 'svg' || format === 'bmp' || format === 'tiff' || format === 'ico') {
                    await axios.put('/api/wn/adjuntos/img', {orden: orden, adjunto: `${original_filename}.${format}`, userEnvia: user, externa: true, url: secure_url});
                } else {
                    await axios.put('/api/wn/adjuntos/scans', {orden: orden, adjunto: `${original_filename}.${format}`, userEnvia: user, externa: true, url: secure_url});
                }
    ...
    return (
        <label className="p-5 bg-green-300 border-2 rounded-xl flex items-center gap-5 hover:bg-green-600 hover:text-white text-xl shadow-xl">
            ...
            <CldUploadWidget 
                ...
                options={{
                    sources: ['local', 'camera', 'google_drive', 'url', 'dropbox'],
                    multiple: true,
                    folder: 'imgs',
                    language: 'es',
                    showPoweredBy: false,
